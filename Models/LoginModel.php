...
public function sessionLogin(int $iduser){
		...
		public function getUserEmail(string $strEmail){
			...
			$sql = "SELECT idpersona,nombres,appat,apmat,status FROM persona WHERE 
					email_user = '$this->strUsuario' and  
					status = 1 ";
			$request = $this->select($sql);
			return $request;
		...
		public function selectValesPe(int $iduserp)
		{
			$this->intIdUsuariov = $iduserp;
			 $sql = "SELECT p.idpersona, p.indentificacion, p.nombres, p.appat, p.apmat,p.direccion, v.idvale , v.monto,v.pago, v.quincenas FROM persona p INNER JOIN vale v ON p.idpersona = v.personaid 
			     WHERE p.idpersona = $this->intIdUsuariov ";
